...
### `constants.py`
This file is the single authoritative source for all physical constants, geometry parameters, and FLUKA dictionaries shared across the module and analysis scripts. This includes the nEXO detector geometry (`OD_RADIUS`, `OD_HEIGHT`, etc.), SNOLAB muon flux parameters, energy bounds, and the FLUKA particle/icode dictionaries (`ICODE_DICTIONARY`, `JTRACK_DICTIONARY`, `NEXO_FLUKA_REGIONS`, etc.). Import from here instead of redefining constants elsewhere.

### `__init__.py`
This is the "init" file. It's basically the place where global variables can be assigned, or declared. I learned this the *hard* way: if you want modifications of a variable in this file from another file to be accessible via yet another file, that variable cannot be a primitive in python. It must be an object. Let me rephrase. If you have `__init__.py`, `a.py`, and `b.py` and some globally accessible variable `v` in `__init__.py`, and you want your change of that variable in `a.py` to be accessible in `b.py`, `v` must be an (pointer to an) object. ANYWAY this file contains pieces that are read in at the beginning of the simulation— system name, seed for the simulation, and so forth. These variables are used elsewhere. Basically anything that needs to be accessed by several of the files in the directory should be put here. Physical constants are imported from `constants.py`.

### `runtools.py`
This file contains commands that interface with the OS for convenient running of the simulation. Basically, it has functions that link and compile the user routines with the fluka binaries, and a command to run the simulation. All OS commands use `subprocess.run(..., check=True)` so that failures raise exceptions rather than being silently ignored.
...
### `analysis.py`
This one is likely to become the largest of the files. When you create a plot that's handy, or you manipulate the data in a useful way, you should save that code somewhere other than a jupyter notebook. This file is meant to house the code that is used for analysis (obviously). It also contains HDF5 file management utilities (`merge_hdf5_files`, `initialize_h5_file`, etc.) previously found in `analysis/analysis_tools.py`.
