...
*                                             ELI Beamlines            *
*                                                                      *
*     Modified on 23 May 2022      by         David Horvath            *
*                                             ELI Beamlines            *
*                                                                      *
...
*=== Source ===========================================================*
      ...
      ! muon_charge = rands(1) !FLRNDM(xdummy)
      ! if(muon_charge .gt. 0.435) then
      !   particle_code = 10
      ! else
      !   particle_code = 11
      ! end if
...
*      momentum_energy = ...
      ...
      !momentum_energy = sample_spectrum("mu_e_hist.txt", "GeV")
...
*      momentum_energy = sample_histogram_momentum_energy( [filename], [unit] )
      ...
      !  momentum_energy = sample_histogram_momentum_energy("mu_e_hist.txt", "GeV")
...
!      do while (.not. hits_roi)

      ! 5. Beam starting position
      ! ========================= 
      ! init_polar = rands(2)*2*pi
      ! init_radius = rands(3)*(gen_radius**2)
      ...
      ! coordinate_x = sqrt(init_radius)*cos(init_polar)
      ! coordinate_y = sqrt(init_radius)*sin(init_polar)
      ! coordinate_z = gen_height
...
*      call sample_annular_distribution( [rmin], [rmax], coordinate_[a], coordinate_[b] )
      ...
      ! direction_cosz = - sample_spectrum('mu_zen_hist.txt', 'MeV') !
      ! direction_cosx = pos_neg_x*rands(6)
      ! direction_cosy = sqrt(1 - direction_cosx**2 - direction_cosz**2) 

      ! MUST GIVE "sample_spectrum" MeV because then scale factor is 1.
...
*      delayed_radioactive_decay = ...
      ...
      ! 8. Sampling from phase space file
      ! =================================
      ...
      call read_phase_space_file('muon_file.txt', 'GeV', 'm', phase_space_entry, .true. , nomore )


      ! 8.5. Reading information from 'phase_space_entry' variable
      ! ----------------------------------------------------------
